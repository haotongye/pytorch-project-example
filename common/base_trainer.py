...
class BaseTrainer:
    def __init__(self, device, cfg, train_data_loader, dev_data_loader, model, losses,
        ...
        if ckpt_path:
            self._start_epoch = self._model.load_state(ckpt_path)
        else:
            self._start_epoch = 0
    ...
    def start(self):
        ...
        bar = tqdm(
            range(self._start_epoch, self._cfg.n_epochs), total=self._cfg.n_epochs,
            initial=self._start_epoch, desc='[Total progress]', leave=False, position=0,
            dynamic_ncols=True)
        for epoch in bar:
            self._stat.reset()
            self._epoch = epoch + 1
